...
void parse_command_line(long long argc, char **argv, char *input_file_name, char *model_file_name);
...
struct problem prob;
struct model* model_;
long long flag_cross_validation;
long long nr_fold;
double bias;
...
void do_cross_validation()
{
	...
	cross_validation(&subprob,&param,nr_fold,target);
...
void parse_command_line(long long argc, char **argv, char *input_file_name, char *model_file_name)
{
	...
	for(i=1;i<argc;i++)
	{
		...
		switch(argv[i-1][1])
		{
			...
			case 'w':
				++param.nr_weight;
				param.weight_label = (long long *) realloc(param.weight_label,sizeof(long long)*param.nr_weight);
...
void read_problem(const char *filename)
{
	long long max_index, inst_max_index, i;
...
//		char *p = strtok(line,","); // label
	...
	prob.numLabels = Malloc(long long ,prob.l);
